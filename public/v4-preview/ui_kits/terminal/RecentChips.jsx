// RecentChips — shows Recent and Pinned page chips under the tape
function RecentChips({ active, setActive, recent, pinned }) {
  // recent: [{id, label}], pinned: [{id, label, prefix?}]
  return (
    <div style={{
      display: 'flex', alignItems: 'center', gap: 18, padding: '8px 18px',
      borderBottom: '1px solid var(--hub-border-subtle)', background: 'var(--hub-black)',
      flexShrink: 0, overflowX: 'auto',
    }}>
      <ChipGroup label="Recent" iconClock items={recent} active={active} setActive={setActive}/>
      <div style={{ width: 1, height: 18, background: 'var(--hub-border-subtle)', flexShrink: 0 }}/>
      <ChipGroup label="Pinned" iconStar items={pinned} active={active} setActive={setActive}/>
      <div style={{ flex: 1 }}/>
      <button style={{
...
function ChipGroup({ label, iconClock, iconStar, items, active, setActive }) {
  return (
        ...
        {items.map(it => {
          ...
          return (
            <button key={it.id} onClick={() => setActive(it.id)} style={{
              ...
              background: on ? 'var(--hub-secondary-medium)' : 'var(--hub-darker)',
              border: '1px solid', borderColor: on ? 'var(--hub-border-hover)' : 'var(--hub-border)',
...
window.RecentChips = RecentChips;
