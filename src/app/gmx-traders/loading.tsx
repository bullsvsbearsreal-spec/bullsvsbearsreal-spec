/**
 * Server-rendered skeleton for the GMX Traders page. Matches the actual
 * layout shape (summary strip + pickers + table + sticky drawer) so the
 * cold-start paint looks like the final UI rather than a blank page.
 */
      ...
      <main className="max-w-[1400px] mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="mb-4">
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            ...
            <div className="ml-auto h-4 w-40 bg-white/[0.06] rounded animate-pulse" />
          </div>
          <div className="h-4 w-[420px] max-w-full bg-white/[0.06] rounded animate-pulse mb-1" />
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          ...
          <div className="ml-auto h-8 w-64 bg-white/[0.06] rounded-lg animate-pulse" />
        </div>

        {/* Table + drawer grid */}
        <div className="grid grid-cols-1 lg:grid-cols-[1fr,380px] gap-4">
          <div className="bg-white/[0.06] rounded-xl p-3 space-y-1.5 min-h-[600px]">
            {Array.from({ length: 12 }, (_, i) => (
              <div key={i} className="h-12 bg-white/[0.06] rounded animate-pulse" />
            ))}
          </div>
          <div className="hidden lg:block h-[480px] bg-white/[0.06] rounded-xl animate-pulse" />
        </div>
      </main>
    </div>
  );
