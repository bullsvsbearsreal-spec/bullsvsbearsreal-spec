...
export default function StablecoinPegError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('[Stablecoin Peg] render error:', error);
  }, [error]);
  ...
  return (
    <main className="max-w-[1400px] mx-auto px-4 sm:px-6 py-20 flex flex-col items-center justify-center text-center">
      ...
      <h1 className="text-lg font-semibold text-white mb-2">Peg monitor hit an issue</h1>
      <p className="text-neutral-400 text-sm mb-6 max-w-sm">
        We couldn&apos;t fetch stablecoin prices. CoinGecko can rate-limit on busy days — a retry usually clears it.
      </p>
