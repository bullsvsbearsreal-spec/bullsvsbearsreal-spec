...
export default function OpenInterestError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('[OpenInterest] render error:', error);
  }, [error]);
  ...
  return (
    <main className="max-w-[1400px] mx-auto px-4 sm:px-6 py-20 flex flex-col items-center justify-center text-center">
      ...
      <h1 className="text-lg font-semibold text-white mb-2">Open interest data unavailable</h1>
      <p className="text-neutral-400 text-sm mb-6 max-w-sm">
        Something went wrong loading OI data. This usually resolves on retry.
      </p>
