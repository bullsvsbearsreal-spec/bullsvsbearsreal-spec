        ...
        <div className="absolute z-50 mt-1 w-48 max-h-64 overflow-auto rounded-lg bg-[#1a1a1a] border border-white/[0.08] shadow-xl">
          <div className="p-2 border-b border-white/[0.06]">
            <div className="flex items-center gap-2 px-2 py-1.5 rounded bg-white/[0.04]">
              ...
              <input value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && filtered.length > 0) { onChange(filtered[0]); setOpen(false); setSearch(''); } if (e.key === 'Escape') { setOpen(false); setSearch(''); } }} placeholder="Search..." aria-label="Search assets" className="bg-transparent text-sm text-white outline-none w-full" autoFocus />
            </div>
          </div>
          {filtered.map(asset => (
