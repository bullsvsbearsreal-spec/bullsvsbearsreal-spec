...
export default function HLTradersError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('[HL Traders] render error:', error);
  }, [error]);
  ...
  return (
    <main className="max-w-[1400px] mx-auto px-4 sm:px-6 py-20 flex flex-col items-center justify-center text-center">
      ...
      <h1 className="text-lg font-semibold text-white mb-2">Hyperliquid Traders hit an issue</h1>
      <p className="text-neutral-400 text-sm mb-6 max-w-sm">
        Something broke loading HL's leaderboard feed. The stats server occasionally throttles — usually clears on retry.
      </p>
