...
export default function AirdropsError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('[Airdrops] render error:', error);
  }, [error]);
  ...
  return (
    <main className="max-w-[1400px] mx-auto px-4 sm:px-6 py-20 flex flex-col items-center justify-center text-center">
      ...
      <h1 className="text-lg font-semibold text-white mb-2">Airdrops failed to load</h1>
      <p className="text-neutral-400 text-sm mb-6 max-w-sm">
        Something went wrong loading airdrop data. This usually resolves on retry.
      </p>
