...
export default function FundingArbError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('[Funding Arb] render error:', error);
  }, [error]);
  ...
  return (
    <main className="max-w-[1400px] mx-auto px-4 sm:px-6 py-20 flex flex-col items-center justify-center text-center">
      ...
      <h1 className="text-lg font-semibold text-white mb-2">Funding arb scan hit an issue</h1>
      <p className="text-neutral-400 text-sm mb-6 max-w-sm">
        The cross-exchange funding aggregation failed. Upstream exchange APIs can rate-limit aggressive scans — retry usually works.
      </p>
