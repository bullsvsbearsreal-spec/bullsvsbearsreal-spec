...
export default function NewsSection({ coinSymbol, limit = 8, compact = false }: NewsSectionProps) {
  ...
  if (!loading && news.length === 0 && coinSymbol) {
    return (
      <div className="bg-hub-gray/20 border border-hub-gray/30 rounded-2xl p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className="text-xl font-bold text-white">{coinSymbol.toUpperCase()} News</h3>
            <p className="text-hub-gray-text text-sm">No recent news found for {coinSymbol.toUpperCase()}</p>
          </div>
        </div>
        <p className="text-neutral-600 text-sm">
          No relevant articles found for this token. Check back later or search on{' '}
          <a href={`https://www.google.com/search?q=${coinSymbol}+crypto+news`} target="_blank" rel="noopener noreferrer" className="text-hub-yellow hover:underline">
            Google News
          </a>.
        </p>
      </div>
