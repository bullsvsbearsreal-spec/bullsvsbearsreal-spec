    ...
    schemas: {
      ...
      FundingRow: {
        ...
        properties: {
          ...
          fundingInterval: { type: 'string', enum: ['1h', '4h', '8h'], description: 'Closest enum bucket for the settlement cadence. Venues that settle outside the 1h/4h/8h cluster (e.g. Blofin = 24h daily-settle) emit the closest bucket here and the precise value via fundingIntervalHours.' },
          fundingIntervalHours: { type: 'number', nullable: true, description: 'Precise per-symbol settlement interval in hours when the venue reports it (1, 2, 4, 8, 24). When present, prefer this over fundingInterval for rate normalization — the enum bucket can lie for 24h venues like Blofin.' },
